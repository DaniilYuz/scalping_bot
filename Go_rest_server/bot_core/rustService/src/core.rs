...
async fn snapshotUpdater(
    receiver: &mut mpsc::Receiver<StreamData>,
    market_snapshot: &Arc<RwLock<MarketSnapshot>>,
    signal_sender: mpsc::Sender<StrategySignal>,
) {
    println!("[DEBUG] snapshotUpdater start");
    ...
    while let Some(msg) = receiver.recv().await {
        ...
        // Запускаем стратегии
        let signals = run_all_strategies(Arc::clone(market_snapshot)).await;

        // Отправляем сигналы обратно через канал
...
async fn run_all_strategies (snapshot: Arc<RwLock<MarketSnapshot>>) -> Vec<StrategySignal> {
    ...
    if let (Some(agg), Some(kline), Some(book)) = (agg_data.clone(), kline_data.clone(), book_ticker_data.clone()) {
        if let Some(signal) = Momentum_strategy(agg, kline, book) {
            signals.push(signal);
        }
    }

    if let (Some(agg), Some(kline)) = (agg_data.clone(), kline_data.clone()) {
        if let Some(signal) = Mean_Reversion(agg, kline) {
            signals.push(signal);
        }
    }

    if let (Some(depth), Some(book)) = (depth_data.clone(), book_ticker_data.clone()) {
        if let Some(signal) = Order_Book_Pressure(depth, book) {
            signals.push(signal);
        }
    }

    if let Some(kline) = kline_data.clone() {
        if let Some(signal) = Volume_spike(kline, &mut snap.volume_history.clone()) {
            signals.push(signal);
        }
    }
...
fn Momentum_strategy (aggTrade: AggTradeData, kline: KlineData, bookTicker: BookTickerData) -> Option<StrategySignal>{
...
fn Mean_Reversion(aggTrade: AggTradeData, kline: KlineData) -> Option<StrategySignal> {
    ...
    else if(devitation_percent > min_deviation_threshold) {
        return Some(StrategySignal {
            tactic_name: "Mean_Reversion".to_string(),
            tactic_signal: "Sell".to_string(),
        });
...
fn Order_Book_Pressure (depth: OrderBookData, book: BookTickerData) -> Option<StrategySignal> {
...
pub async fn run_trading_bot(
    coins: Vec<String>,                 // монеты для подписки
    output: mpsc::Sender<StrategySignal>, // канал для Manager
) -> Result<(), String> {
    ...
    // --- 2. Канал для данных с WebSocket ---
    let (ws_sender, mut ws_receiver) = mpsc::channel::<StreamData>(5000);

    // --- 3. Snapshot updater ---
    let snapshot_updater_handle = {
        let snapshot_clone = Arc::clone(&snapshot);
        let output_clone = output.clone(); // Клон канала для отправки сигналов
        tokio::spawn(async move {
            snapshotUpdater(&mut ws_receiver, &snapshot_clone, output_clone).await;
        })
    };


    // --- 4. Формируем эндпоинты для WebSocket ---
    let endpoints: Vec<String> = coins.iter().flat_map(|coin| {
        ["arr", "kline_1m", "depth", "bookTicker"].iter().map(move |stream| {
            let formatted = format!("{}@{}", coin.to_lowercase(), stream);
            if formatted.contains("@arr") { "!ticker@arr".to_string() } else { formatted }
        })
    }).collect();
    ...
    let websocket_handle = tokio::spawn(async move {
        let mut ws = WebSockets::new(move |event: WebsocketEvent| {
            handle_event_AggTrade(&event, &ws_sender_clone, &coins_clone).unwrap_or(());
            handle_event_Kline(&event, &ws_sender_clone).unwrap_or(());
            handle_event_DepthOrderBook(&event, &ws_sender_clone).unwrap_or(());
            handle_event_BookTicker(&event, &ws_sender_clone).unwrap_or(());
            Ok(())
        });

        if let Err(e) = ws.connect_multiple_streams(&endpoints) {
            println!("WebSocket connection error: {:?}", e);
            return;
        }

        if let Err(e) = ws.event_loop(&keep_running_ws) {
            println!("WebSocket event_loop error: {:?}", e);
        }
    ...
    // --- 7. Запуск стратегий ---
    let snapshot_clone2 = Arc::clone(&snapshot);
    let keep_running_strategy = keep_running.clone();
    let output_clone = output.clone();
    let strategy_handle = tokio::spawn(async move {
        while keep_running_strategy.load(Ordering::Relaxed) {
            let signals = run_all_strategies(snapshot_clone2.clone()).await;
            for sig in signals {
                let _ = output_clone.send(sig).await;
            }
